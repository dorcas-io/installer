Vue.component("webstore-product", {
  template:
    '<div class="product clearfix" v-bind:class="[customClass]">' +
  ...
  computed: {
    ...
    default_price: function() {
      if (
        typeof this.product.prices === "undefined" ||
        this.product.prices.data.length === 0
      ) {
        return {
          currency: this.currency,
          unit_price: {
            raw: product.default_unit_price.raw,
            formatted: product.default_unit_price.formatted
          }
        };
      }
  ...
  methods: {
    addToCart: function() {
      if (this.product_json.inventory === 0) {
        swal(
          "Out of Stock",
          "We're sorry but this product is out of stock at the moment."
        );
        return false;
      }
      this.is_processing = true;
      var image =
        typeof this.product.images !== "undefined" &&
        this.product.images.data.length > 0
          ? this.product.images.data[0].url
          : "";
      this.$emit(
        "add-to-cart",
        this.product.id,
        this.product.name,
        this.default_price.unit_price.raw,
        image
      );
    }
  }
