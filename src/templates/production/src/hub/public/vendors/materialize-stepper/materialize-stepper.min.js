...
($.fn.isValid = function() {
  ...
  validation &&
    $.validator.setDefaults({
      ...
      errorPlacement: function(a, b) {
        b.is(":radio") || b.is(":checkbox")
          ? a.insertBefore($(b).parent())
          : a.insertAfter(b);
      },
      success: function(a) {
        $(a)
  ...
  ($.fn.getActiveStep = function() {
    return (
      ...
      $(this.children(".step:visible")).index($(active)) + 1
    );
  }),
  ($.fn.activateStep = function(a) {
    $(this).hasClass("step") ||
      ...
      window.innerWidth < 993 || !stepper.hasClass("horizontal")
        ? $(this)
            .addClass("step")
            .stop()
  ...
  ($.fn.showError = function(a) {
    if (validation) {
      ...
      var b = {};
      (b[name] = a),
        form.validate().showErrors(b),
        this.closest("li").addClass("wrong");
    } else
      this.removeClass("valid").addClass("invalid"),
        this.next().attr("data-error", a);
  }),
  ...
  ($.fn.openStep = function(a, b) {
    (settings = $(this)
      ...
      ($this = this),
      (step_num = a - 1),
      (a = this.find(".step:visible:eq(" + step_num + ")")),
      a.hasClass("active") ||
        ((active = this.find(".step.active")),
        (prev_active = next = $(this.children(".step:visible")).index(
          $(active)
        )),
        (order = step_num > prev_active ? 1 : 0),
        active.hasClass("feedbacking") && $this.destroyFeedback(),
        active.closeAction(order),
        a.openAction(order, function() {
          settings.autoFocusInput &&
            a.find("input:enabled:visible:first").focus(),
            $this.trigger("stepchange").trigger("step" + (step_num + 1)),
            a.data("event") && $this.trigger(a.data("event")),
            b && b();
        }));
  }),
  ($.fn.closeAction = function(a, b) {
    (closable = this.removeClass("active").find(".step-content")),
      window.innerWidth < 993 || !this.closest("ul").hasClass("horizontal")
        ? closable.stop().slideUp(300, "easeOutQuad", b)
        : 1 == a
        ? closable.animate({ left: "-100%" }, function() {
            closable.css({ display: "none", left: "0%" }, b);
          })
        : closable.animate({ left: "100%" }, function() {
            closable.css({ display: "none", left: "0%" }, b);
          });
  ...
  ($.fn.openAction = function(a, b) {
    (openable = this.removeClass("done")
      .addClass("active")
      .find(".step-content")),
      window.innerWidth < 993 || !this.closest("ul").hasClass("horizontal")
        ? openable.slideDown(300, "easeOutQuad", b)
        : 1 == a
        ? openable
  ...
  ($.fn.activateStepper = function(a) {
    ...
    $(document).on("click", function(a) {
      $(a.target).parents(".stepper").length ||
      ...
      $(this).each(function() {
        ...
        !a.parents("form").length &&
          ...
          a.wrap(
            '<form action="' + action + '" method="' + method + '"></div>'
          )),
          a.data("settings", {
          ...
          a.find("li.step.active").openAction(1),
          a.find(">li").removeAttr("data-last"),
          a
            .find(">li.step")
            .last()
            .attr("data-last", "true"),
          a
            .on("click", ".step:not(.active)", function() {
              (object = $(a.children(".step:visible")).index($(this))),
                a.hasClass("linear")
                  ? b.linearStepsNavigation &&
                    ((active = a.find(".step.active")),
                    $(a.children(".step:visible")).index($(active)) + 1 ==
                    object
                      ? a.nextStep(void 0, !0, void 0)
                      : $(a.children(".step:visible")).index($(active)) - 1 ==
                          object && a.prevStep(void 0))
                  : a.openStep(object + 1);
            })
            ...
            .on(
              ...
              function(b) {
                if (
                  (b.preventDefault(),
                  (feedback = b ? $(b.target).data("feedback") : void 0),
                  (form = a.closest("form")),
                  form.isValid())
                ) {
                  if (feedback)
                    return stepper.activateFeedback(), window[feedback].call();
                  form.submit();
                }
              }
            )
