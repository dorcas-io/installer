(function(angular) {
  ...
  angular.module("ui.materialize.ngModel", []).directive("ngModel", [
    "$timeout",
    function($timeout) {
      return {
        restrict: "A",
  ...
  /* example usage: 
    ...
    */
  angular.module("ui.materialize.slider", []).directive("slider", [
    "$timeout",
    function($timeout) {
      return {
        restrict: "A",
        ...
        link: function(scope, element, attrs) {
          ...
          $timeout(function() {
            element.slider({
              ...
              transition: angular.isDefined(scope.transition)
                ? scope.transition
                : 500,
              interval: angular.isDefined(scope.interval)
                ? scope.interval
                : 6000,
              indicators: angular.isDefined(scope.indicators)
                ? scope.indicators
                : true
            });
          });
        ...
        return {
          ...
          link: function(scope, element, attrs) {
            element.bind(attrs.toast, function() {
              var message = angular.isDefined(scope.message)
                ? scope.message
                : "";
              var rounded = angular.isDefined(attrs.rounded)
                ? toastConfig.rounded
                : null;
              Materialize.toast(
                message,
                scope.duration ? scope.duration : toastConfig.duration,
                rounded
              );
            });
          }
  ...
  angular.module("ui.materialize.pushpin", []).directive("pushpin", [
    function() {
      return {
        restrict: "AE",
  ...
  angular.module("ui.materialize.tabs", []).directive("tabs", [
    function() {
      return {
        link: function(scope, element, attrs) {
          element.tabs();
  ...
  angular.module("ui.materialize.sidenav", []).directive("sidenav", [
    function() {
      return {
        scope: {
          menuwidth: "@",
        ...
        link: function(scope, element, attrs) {
          element.sideNav({
            menuWidth: angular.isDefined(scope.menuwidth)
              ? scope.menuwidth
              : undefined,
            edge: attrs.sidenav ? attrs.sidenav : "left",
            closeOnClick: angular.isDefined(scope.closeonclick)
  ...
  angular.module("ui.materialize.dropdown", []).directive("dropdown", [
    "$compile",
    "$timeout",
    function($compile, $timeout) {
      return {
        ...
        link: function(scope, element, attrs) {
          $timeout(function() {
            $compile(element.contents())(scope);
            element.dropdown({
              inDuration: angular.isDefined(scope.inDuration)
                ? scope.inDuration
                : undefined,
              outDuration: angular.isDefined(scope.outDuration)
                ? scope.outDuration
                : undefined,
              constrain_width: angular.isDefined(scope.constrainWidth)
                ? scope.constrainWidth
                : undefined,
              hover: angular.isDefined(scope.hover) ? scope.hover : undefined,
              alignment: angular.isDefined(scope.alignment)
                ? scope.alignment
                : undefined,
              gutter: angular.isDefined(scope.gutter)
                ? scope.gutter
                : undefined,
              belowOrigin: angular.isDefined(scope.belowOrigin)
                ? scope.belowOrigin
                : undefined
            });
          });
  ...
  /**
     ...
     */
  angular.module("ui.materialize.inputfield", []).directive("inputField", [
    "$compile",
    "$timeout",
    function($compile, $timeout) {
      return {
        ...
        link: function(scope, element) {
          $timeout(function() {
            ...
            element
              .find("> > .materialize-textarea, > > input")
              .each(function(index, countable) {
                countable = angular.element(countable);
                if (
                  !countable.siblings('span[class="character-counter"]').length
                ) {
  ...
  /**
     ...
     * Example:
     <input input-date
        ...
        on-stop="onStop()" />
     */
  angular.module("ui.materialize.input_date", []).directive("inputDate", [
    "$compile",
    "$timeout",
    function($compile, $timeout) {
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
          ngModelCtrl.$formatters.unshift(function(modelValue) {
            if (modelValue) {
              ...
              return angular.isDefined(scope.format)
          ...
          var monthsFull = angular.isDefined(scope.monthsFull)
              ? scope.$eval(scope.monthsFull)
              : undefined,
            monthsShort = angular.isDefined(scope.monthsShort)
              ? scope.$eval(scope.monthsShort)
              : undefined,
            weekdaysFull = angular.isDefined(scope.weekdaysFull)
              ? scope.$eval(scope.weekdaysFull)
              : undefined,
            weekdaysLetter = angular.isDefined(scope.weekdaysLetter)
              ? scope.$eval(scope.weekdaysLetter)
              : undefined;
          ...
          if (!scope.ngReadonly) {
            $timeout(function() {
              var pickadateInput = element.pickadate({
                container: angular.isDefined(scope.container)
                  ? scope.container
                  : "body",
                format: angular.isDefined(scope.format)
                  ? scope.format
                  : undefined,
                formatSubmit: angular.isDefined(scope.formatSubmit)
                  ? scope.formatSubmit
                  : undefined,
                monthsFull: angular.isDefined(monthsFull)
                  ? monthsFull
                  : undefined,
                monthsShort: angular.isDefined(monthsShort)
                  ? monthsShort
                  : undefined,
                weekdaysFull: angular.isDefined(weekdaysFull)
                  ? weekdaysFull
                  : undefined,
                weekdaysLetter: angular.isDefined(weekdaysLetter)
                  ? weekdaysLetter
                  : undefined,
                disable: angular.isDefined(scope.disable)
                  ? scope.disable
                  : undefined,
                ...
                selectYears: angular.isDefined(scope.selectYears)
                  ? scope.selectYears
                  : undefined,
                onStart: angular.isDefined(scope.onStart)
                  ? function() {
                      scope.onStart();
                    }
                  : undefined,
                onRender: angular.isDefined(scope.onRender)
                  ? function() {
                      scope.onRender();
                    }
                  : undefined,
                onOpen: angular.isDefined(scope.onOpen)
                  ? function() {
                      scope.onOpen();
                    }
                  : undefined,
                onClose: angular.isDefined(scope.onClose)
                  ? function() {
                      scope.onClose();
                    }
                  : undefined,
                onSet: angular.isDefined(scope.onSet)
                  ? function() {
                      scope.onSet();
                    }
                  : undefined,
                onStop: angular.isDefined(scope.onStop)
                  ? function() {
                      scope.onStop();
                    }
                  : undefined
    ...
    .directive("pagination", function() {
      ...
      function addPrevNext(scope, pageCount, mode) {
        ...
        if (mode === "prev") {
          ...
          var nextPage =
            scope.page + 1 >= pageCount ? pageCount : scope.page + 1;
  ...
  /*     example usage:
     ...
     */
  angular.module("ui.materialize.modal", []).directive("modal", [
    "$compile",
    "$timeout",
    function($compile, $timeout) {
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          $timeout(function() {
            element.leanModal({
              dismissible: angular.isDefined(scope.dismissible)
                ? scope.dismissible
                : undefined,
              opacity: angular.isDefined(scope.opacity)
                ? scope.opacity
                : undefined,
              in_duration: angular.isDefined(scope.inDuration)
                ? scope.inDuration
                : undefined,
              out_duration: angular.isDefined(scope.outDuration)
                ? scope.outDuration
                : undefined
            });
          });
  ...
  /*     example usage:
     ...
     */
  angular
    .module("ui.materialize.materialboxed", [])
    .directive("materialboxed", [
      "$timeout",
      function($timeout) {
        return {
          restrict: "A",
