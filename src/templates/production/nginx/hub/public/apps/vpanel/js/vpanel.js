  ...
  formatters: {
    companies: function(row, index) {
      ...
      if (typeof row.contact !== "undefined") {
        contact_name +=
          row.contact.firstname !== null ? row.contact.firstname + " " : "";
        contact_name +=
          row.contact.lastname !== null ? row.contact.lastname : "";
      }
      row.status = row.is_trashed ? "Trashed" : "Live";
      row.menu =
        '<div class="actions" style="font-size: 1.2rem; margin-left: 10px;">';
      if (!row.is_trashed) {
        row.menu +=
          '<a href="/vpanel/businesses/' +
          row.id +
          '" data-action="view"><i class="ion-md-eye"></i></a>' +
          '&nbsp;<a href="/vpanel/businesses/' +
          row.id +
          '/edit" data-action="view"><i class="ion-md-create"></i></a>';
      }
      row.menu +=
        '&nbsp;<a href="#"><i  data-action="delete" data-index="' +
        index +
    ...
    invites: function(row, index) {
      if (
        typeof row.inviting_user !== "undefined" &&
        row.inviting_user.data !== null
      ) {
        row.invited_by =
          row.inviting_user.data.firstname +
          " " +
          row.inviting_user.data.lastname;
      } else if (
        typeof row.inviter !== "undefined" &&
        row.inviter.data !== null
      ) {
        row.invited_by =
          row.inviter.data.firstname + " " + row.inviter.data.lastname;
      }
      row.action = row.config_data.action
        .split("_")
        .join(" ")
        .title_case();
...
Vue.component("address-table-row", {
  ...
  computed: {
    ...
    fullAddress: function() {
      ...
      address +=
        this.address.address2 !== null && this.address.address2.length > 0
          ? " " + this.address.address2.toUpperCase()
          : "";
      address +=
        this.address.city !== null && this.address.city.length > 0
          ? ", " + this.address.city.toUpperCase()
          : "";
      address +=
        typeof this.address.state !== "undefined"
          ? ", " + this.address.state.data.name.toUpperCase()
          : "";
      return address;
    }
  },
  methods: {
    deleteAddress: function() {
      ...
      swal({
        ...
        axios
          .delete("/xhr/company/addresses/" + context.address.id)
          .then(function(response) {
            console.log(response);
            context.$emit("deleted", context.index);

            return vpanel_simple_swal(
              "Deleted!",
...
Vue.component("vehicle-table-row", {
  template:
    "<tr v-bind:class=\"{'selected': vehicle.is_checked}\">" +
  ...
  mounted: function() {
    ...
    if (this.model !== null) {
      this.make =
        typeof this.model.vehicle_make !== "undefined"
          ? this.model.vehicle_make.data
          : null;
      this.type =
        typeof this.model.vehicle_type !== "undefined"
          ? this.model.vehicle_type.data
          : null;
    }
  },
  methods: {
    ...
    deleteVehicle: function() {
      ...
      swal({
        ...
        axios
          .delete("/xhr/vehicles/" + context.vehicle.id)
          .then(function(response) {
            console.log(response);
            context.$emit("deleted", context.index);
            return vpanel_simple_swal(
  ...
  methods: {
    deleteInsurance: function() {
      ...
      swal({
        ...
        title: "Are you sure?",
        text:
          "You are about to delete this insurance record: " +
          context.insurance.policy_number,
        customClass: "swal2-btns-left",
        showCancelButton: true,
        confirmButtonClass: "swal2-btn swal2-btn-confirm",
        confirmButtonText: "Yes, delete it!",
        ...
        axios
          ...
          .then(function(response) {
            ...
            return vpanel_simple_swal(
              "Deleted!",
              "The insurance record was successfully deleted.",
              "success"
            );
          })
          .catch(function(error) {
  ...
  methods: {
    ...
    deletePolicy: function() {
      ...
      swal({
        ...
        title: "Are you sure?",
        text:
          "You are about to delete this insurance policy: " +
          context.policy.policy_number,
        customClass: "swal2-btns-left",
        showCancelButton: true,
        confirmButtonClass: "swal2-btn swal2-btn-confirm",
        confirmButtonText: "Yes, delete it!",
        ...
        axios
          .delete("/xhr/insurance-companies/" + context.policy.id)
          .then(function(response) {
            console.log(response);
            context.$emit("deleted", context.index);
