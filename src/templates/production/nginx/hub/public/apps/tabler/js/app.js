var app = {
  ...
  utilities: {
    ...
    roleNamesFromArray: function(user) {
      ...
      return user.roles.map(function(e) {
        return v.titleCase(e);
      });
    }
  },
  formatters: {
    access_grants: function(row, index) {
      let user =
        typeof row.user !== "undefined" && row.user.data !== null
          ? row.user.data
          : null;
      if (user !== null) {
        row.avatar =
      ...
      row.modules_count =
        typeof row.extra_json.modules !== "undefined"
          ? row.extra_json.modules.length
          : 0;
      row.pending_modules_count =
        typeof row.extra_json.pending_modules !== "undefined"
          ? row.extra_json.pending_modules.length
          : 0;
      row.menu = "";
      if (app.currentUser.uuid !== row.id) {
        row.menu = '<div style="font-size: 1.2rem; margin-left: 10px;">';
        row.menu +=
          '<a href="#" class="btn btn-icon"><i data-action="edit" data-index="' +
          index +
          '"  class="fe fe-eye"></i></a>';
        row.menu +=
          '<a href="#" class="btn btn-icon"><i  data-action="delete" data-index="' +
          index +
